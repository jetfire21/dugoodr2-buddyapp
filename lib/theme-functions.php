...
if ( ! function_exists( 'kleo_entry_meta' ) ) {
	function kleo_entry_meta( $args = array() )
	{
		...
		$classes = apply_filters( 'kleo_entry_meta_classes', $args['class'] );

		$output = '';
		$meta_elements =  sq_option( 'blog_meta_elements', Kleo::get_config('blog_meta_defaults') );

		if ( is_array( $meta_elements ) && ! empty( $meta_elements ) ) {


			$output = '<' . $args['container'] . ' class="' . esc_attr( $classes ) . '">';
			...
			if ( in_array( 'date', $meta_elements ) ) {
				...
				$output .= '<' . $args['entry_tag'] . '><i class="icon-calendar"></i> ' . $date . '</' . $args['entry_tag'] . '>';
			}
